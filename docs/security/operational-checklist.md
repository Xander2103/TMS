...
| 2 | Productie-secretbron (JWT key, connstring, webhook secret) | Config leeg; bron onbekend | Vault/env buiten repo | Bevestig secrets uit key vault/env; geen `appsettings.*` met echte waarden in publish. Webhook: zet minimaal `Peppol:Webhook:Secret`; configureer bij een provider die signing ondersteunt `Peppol:Webhook:Hmac:Secret` (verplicht replay-window, default 300 s); rotatie via `PreviousSecrets`; per-provider scoping via `Peppol:Webhook:Providers:{key}` | Ops/Sec | **Ja** |

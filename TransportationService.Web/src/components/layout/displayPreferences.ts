 ...
 *
 * The gate COMPONENT lives in `DisplayPreferencesProvider.tsx`; this module owns everything
 * non-component (react-refresh: a component file exports only components).
 */
 ...
 * (AppLayout applies `defaultLanguage` as the locale fallback); the gate component uses it to
 * decide when the routed content may render. Everything is scoped to the current session key, so
 * a session change closes the gate again instead of showing the previous tenant's clock.
 */

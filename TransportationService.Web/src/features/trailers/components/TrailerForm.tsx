  ...
  const sections: SectionDef[] = [
    {
      ...
      render: () => (
        <FormSection title="Algemeen" columns={3}>
          <FormField label="Kenteken" htmlFor="tf-plate" required error={fieldErrors.licensePlate}>
            <input
              id="tf-plate"
          ...
          <FormField label="Categorie" htmlFor="tf-category">
            <LookupSelect
              id="tf-category"
          ...
          <FormField label="Merk" htmlFor="tf-brand">
          ...
          <FormField label="Eigendomsvorm" htmlFor="tf-ownership">
          ...
          {mode === 'edit' && (
            <>
              <FormField label="Operationele status" htmlFor="tf-status">
                <select
                  id="tf-status"
              ...
              {form.operationalStatus !== 'Available' && (
                <FormField label="Reden" htmlFor="tf-status-reason">
    ...
    },
    {
      ...
      render: () => (
        <FormSection title="Registratie" columns={3}>
          <FormField label="Chassisnummer (VIN)" htmlFor="tf-vin">
            <input id="tf-vin" value={form.vin ?? ''} onChange={(e) => set('vin', e.target.value || null)} disabled={isSubmitting} maxLength={50} />
          </FormField>
          <FormField label="Bouwjaar" htmlFor="tf-year" error={fieldErrors.year}>
            <input
              id="tf-year"
              type="number"
              min={1900}
          ...
          <FormField label="Eerste registratie" htmlFor="tf-firstreg">
    ...
    },
    {
      ...
      render: () => (
        <FormSection title="Capaciteit & afmetingen" columns={3}>
          <FormField label="Laadvermogen (kg)" htmlFor="tf-capacity">
          ...
          <FormField label="Aantal assen" htmlFor="tf-axles" hint="Voor Maut/tolberekening.">
          ...
          <FormField label="Laadmeters" htmlFor="tf-ldm">
          ...
          <FormField label="Lengte (m)" htmlFor="tf-length">
          ...
          <FormField label="Breedte (m)" htmlFor="tf-width">
          ...
          <FormField label="Hoogte (m)" htmlFor="tf-height">
    ...
    },
    {
      ...
      optional: true,
      render: () => (
        <FormSection title="Techniek" columns={1}>
        ...
        <FormSection title="Onderhoud & keuringen" columns={1}>
          {maintenanceSection ?? (
            <p className="placeholder-text">
              ...
              (opleggerregel → categorieregel → bedrijfsstandaard).
            </p>
          )}
        </FormSection>
      ),
    },
    {
      ...
      render: () => (
        <FormSection title="Notities" columns={1}>
          <FormField label="Interne notities" htmlFor="tf-notes" className="form-span-all">
        ...
        actions={
          <FormActions dirty={dirty}>
            ...
            <Button type="submit" disabled={isSubmitting}>
              {isSubmitting ? 'Bezig…' : mode === 'create' ? 'Oplegger aanmaken' : 'Opslaan'}
            </Button>
          </FormActions>
        }
      />
